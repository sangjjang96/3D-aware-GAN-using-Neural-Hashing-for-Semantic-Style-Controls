...
def train(opt, experiment_opt, rendering_opt, loader, generator, g_discriminator, s_discriminator, g_optim, gd_optim, sd_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        # real_imgs, real_masks, _ = next(loader)
        ...
        d_loss.backward()
        # torch.nn.utils.clip_grad_norm_(g_discriminator.parameters(), 10)
        gd_optim.step()
        ...
        sd_loss.backward()
        # torch.nn.utils.clip_grad_norm_(s_discriminator.parameters(), 10)
        sd_optim.step()
        ...
        for j in range(0, opt.batch, opt.chunk):
            ...
            fake_img  = out[1]          # fake_img11
            if opt.min_surf_lambda > 0:
                sdf = out[2]
            if opt.eikonal_lambda > 0:
                eikonal_term = out[3]
            fake_seg = out[-1]          # mask11

            # with torch.no_grad():
            #     out2, latent2, latent_global2 = generator(noise2, cam_extrinsics, focal, near, far,
            #                     lat = True)   
            #     fake_img22 = out2[1]          # fake_img22
            #     fake_seg22 = out2[-1]          # mask22

            #     lat1 = latent1
            #     lat2 = latent2
            #     lat1_glo = latent_global1
            #     lat2_glo = latent_global2
                
            #     for s in range(9):
            #         lat1[(s*5)+3:(s*5)+5] = latent2[(s*5)+3:(s*5)+5]
            #         lat2[(s*5)+3:(s*5)+5] = latent1[(s*5)+3:(s*5)+5]

            #     noise12 = [lat1, lat1_glo]
            #     out12 = generator(noise12, cam_extrinsics, focal, near, far,
            #                     std = True)
            #     fake_img12 = out12[1]          # fake_img12
            #     fake_seg12 = out12[-1]          # mask12

            #     noise21 = [lat2, lat2_glo]
            #     out21 = generator(noise21, cam_extrinsics, focal, near, far,
            #                     std = True)
            #     fake_img21 = out21[1]          # fake_img21
            #     fake_seg21 = out21[-1]          # mask21
            ...
            # std_loss = decouple_loss(fake_seg, fake_seg12, fake_img, fake_img21, fake_img22)
        ...
        # g_std_val = loss_reduced["g_std"].mean().item()
        ...
        if get_rank() == 0:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    step_size = 2
                    mean_latent = g_module.mean_latent(1000, device)
                    for k in range(0, opt.val_n_sample * 8, step_size):
                        _, curr_samples, curr_seg = g_ema([sample_z[0][k:k+step_size]],     # original = [sample_z[0]]    modified = sample_z
            ...
            if wandb and opt.wandb:
                wandb_log_dict = {
                                  ...
                                  "G eikonal loss": g_eikonal_val,
                                  "G minimal surface loss": g_minimal_surface_val,
                                #   "G std loss": g_std_val,
