...
def decouple_loss(mask11, mask12, img11, img21, img22):
    use_pretrained=True
    vgg = models.vgg16(pretrained=use_pretrained)
    vgg_features = vgg.features
    vgg_features.eval()
    mask_loss = F.l1_loss(mask11, mask12)
    shape_loss = F.l1_loss(vgg_features(img11.detach().cpu()), vgg_features(img21.detach().cpu()))
    texture_loss = F.l1_loss(vgg_features(img11.detach().cpu()), vgg_features(img22.detach().cpu()))
    margin = 1e-8
    
    st_loss = shape_loss - texture_loss + margin if shape_loss - texture_loss + margin > 0 else 0

    return mask_loss + st_loss