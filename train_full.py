...
from generators.model import Generator, Discriminator
from datasets import CelebA, color_segmap
# from dataset import MultiResolutionDataset, color_segmap
from generators.utils import data_sampler, requires_grad, accumulate, sample_data, make_noise, mixing_noise, generate_camera_params
...
def train(opt, experiment_opt, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        real_imgs, _, real_segs = next(loader)
        # real_imgs, real_segs, _ = next(loader)
        ...
        for j in range(0, opt.batch, opt.chunk):
            ...
            curr_real_segs = real_segs[j:j+opt.chunk]
            ...
            gen_imgs, gen_segs = out[0]

            fake_pred = discriminator(gen_imgs.detach(), gen_segs.detach())

            if d_regularize:
                curr_real_imgs.requires_grad = True
                curr_real_segs.requires_grad = True

            real_pred = discriminator(curr_real_imgs, curr_real_segs)
            d_gan_loss = d_logistic_loss(real_pred, fake_pred)

            if d_regularize:
                grad_penalty_img, grad_penalty_seg = d_r1_loss(real_pred, curr_real_imgs, curr_real_segs)

                r1_loss = (opt.r1_img * 0.5 * grad_penalty_img + opt.r1_seg * 0.5 * grad_penalty_seg) * opt.d_reg_every
            else:
        ...
        if g_regularize:
            ...
            for j in range(0, path_batch_size, opt.chunk):
                path_fake_img, path_fake_seg, path_latents = generator(path_noise, path_cam_extrinsics,
                                                        path_focal, path_near, path_far,
                                                        return_latents=True)
                if i == 0 and j == 0:
                    mean_path_length = torch.zeros([], device=path_latents.device)
                ...
                if opt.path_batch_shrink:
                    weighted_path_loss += 0 * path_fake_img[0, 0, 0, 0] + 0 * path_fake_seg[0, 0, 0, 0]
        ...
        if get_rank() == 0:
            ...
            if i % 1000 == 0 or i == opt.start_iter:
                with torch.no_grad():
                    thumbs_samples = torch.Tensor(0, 3, opt.renderer_output_size, opt.renderer_output_size * 2)
                    samples = torch.Tensor(0, 3, opt.size, opt.size * 2)
                    step_size = 8
                    ...
                    for k in range(0, opt.val_n_sample * 8, step_size):
                        ...
                        (curr_samples, curr_seg_high), curr_thumbs, curr_segs = out[0], out[1], out[-1]

                        samples = torch.cat([samples, torch.cat([(curr_samples.clamp(-1,1).cpu()+1)/2.0 * 255.0, color_segmap(curr_seg_high).cpu()], dim=-1)], 0)
                        thumbs_samples = torch.cat([thumbs_samples, torch.cat([(curr_thumbs.cpu()+1)/2.0 * 255.0, color_segmap(curr_segs).cpu()], dim=-1)], 0)
    ...
    if get_rank() == 0:
        torch.save(
            ...
            os.path.join(opt.checkpoints_dir, experiment_opt.expname, experiment_opt.expname + '_full_pipeline.pt')
        )
...
if __name__ == "__main__":
    ...
    for key, value in params_dict_g.items():
        decoder_cond = ('decoder' in key or 'feature_transfer' in key)
        if decoder_cond:
    ...
    d_optim = optim.Adam(discriminator.parameters(),
                         lr=opt.training.lr * d_reg_ratio,
    ...
    if not opt.experiment.continue_training:
        ...
        if opt.training.pretrain_render_path == '':
            pretrained_renderer_path = os.path.join(opt.training.checkpoints_dir, opt.experiment.expname, opt.experiment.expname + '_vol_renderer.pt')
        else:
            pretrained_renderer_path = os.path.join(opt.training.pretrain_render_path)
