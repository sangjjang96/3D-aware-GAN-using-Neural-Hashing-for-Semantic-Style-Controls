...
class NoiseInjection(nn.Module):
    ...
    def project_noise(self, noise, transform, mesh_path=None):
        ...
        depth_max = depth.max(-1)[0].view(-1)
        ...
        noise_copy = noise_copy.reshape(1,1,height,height)
...
class Decoder(nn.Module):
    def __init__(self, model_opt, blur_kernel=[1, 3, 3, 1]):
        ...
        self.style_dim = model_opt.style_dim * 2 # 512
        ...
        decoder_in_size = model_opt.renderer_spatial_output_dim # 64

        # image decoder
        self.log_size = int(math.log(self.size, 2)) # 9
        self.log_in_size = int(math.log(decoder_in_size, 2)) # 6
        ...
        self.to_rgb1 = ToRGB(self.channels[decoder_in_size], 3,  self.style_dim, upsample=False)
        self.to_seg1 = ToRGB(self.channels[decoder_in_size], model_opt.n_wcode, self.style_dim, upsample=False)
        ...
        for i in range(self.log_in_size+1, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(out_channel, 3, self.style_dim))
            self.to_segs.append(ToRGB(out_channel, model_opt.n_wcode, self.style_dim))
    ...
    def forward(self, features, styles, rgbd_in=None, seg_in=None, transform=None,
        ...
        skip_seg = self.to_seg1(out, latent[:, 1], skip=seg_in)
        ...
        for conv1, conv2, noise1, noise2, to_rgb, to_seg in zip(
            ...
            skip_seg = to_seg(out, latent[:, i + 2], skip=skip_seg)
...
class Generator(nn.Module):
    def __init__(self, model_opt, renderer_opt, blur_kernel=[1, 3, 3, 1], ema=False, full_pipeline=True):
        ...
        if self.full_pipeline:
            self.feature_transfer = ConvLayer(renderer_opt.width, 256, 3)
    ...
    def expand_latents(self, latent):
        ...
        for i in range(self.n_branch):
            latent_expanded.append( latent[:,2*i].unsqueeze(1).repeat(1,self.shape_n,1) )
            latent_expanded.append( latent[:,2*i+1].unsqueeze(1).repeat(1,self.texture_n,1) )
    ...
    def styles_and_noise_forward(self, styles, inject_index=None, truncation=1,
                                 truncation_latent=None, input_is_latent=False, mix_styles=True):
        # for s in styles:
    ...
    def init_forward(self, styles, cam_poses, focals, near=0.88, far=1.12):
        latent, _ = self.styles_and_noise_forward(styles)
        sdf, target_values = self.renderer.mlp_init_pass(cam_poses, focals, near, far, styles=latent)
    ...
    def forward(self, styles, cam_poses, focals, near=0.88, far=1.12, return_latents=False,
                ...
                project_noise=False, mesh_path=None, mix_styles=True, styles_global=None, semantics=None, lat=False, std=False,):

        with torch.set_grad_enabled(self.is_train and self.train_renderer):

            if std:
                latent, latent_global = styles
            else:
                latent, latent_global = self.styles_and_noise_forward(styles, inject_index, truncation,
                                                        truncation_latent, input_is_latent, mix_styles)

            thumb_rgb, features, sdf, mask, xyz, eikonal_term, seg = self.renderer(cam_poses, focals, near, far, styles=latent, return_eikonal=return_eikonal, semantics=semantics)

        if self.full_pipeline:
            features = self.feature_transfer(features)
            if styles_global is not None:
                latent_global = styles_global
            rgb, seg_high, decoder_latent = self.decoder(features, latent_global, seg_in=seg,
                                               ...
                                               input_is_latent=False, randomize_noise=randomize_noise,
                                               mesh_path=mesh_path)
        ...
        if return_latents:
            return rgb, seg_high, decoder_latent
        elif lat:
            out = ((rgb, seg_high), thumb_rgb)
            if return_xyz:
                out += (xyz,)
            if return_sdf:
                out += (sdf,)
            ...
            return out, latent, latent_global 
        else:
            out = ((rgb, seg_high), thumb_rgb)
            if return_xyz:
                out += (xyz,)
...
class VolumeRenderDiscriminator(nn.Module):
    def __init__(self, opt):
        super().__init__()
        init_size = opt.renderer_spatial_output_dim # 64
        ...
        ### img
        convs_img = [VolumeRenderDiscConv2d(3, channels[init_size], 1, activate=True)]
        log_size = int(math.log(init_size, 2))
        in_channel = channels[init_size]

        for i in range(log_size-1, 0, -1):
            out_channel = channels[2 ** i]
            convs_img.append(VolumeRenderResBlock(in_channel, out_channel))
            in_channel = out_channel

        self.convs_img = nn.Sequential(*convs_img)

        ### seg
        convs_seg = [VolumeRenderDiscConv2d(opt.n_wcode, channels[init_size], 1, activate=True)]
        log_size = int(math.log(init_size, 2))
        in_channel = channels[init_size]

        for i in range(log_size-1, 0, -1):
            out_channel = channels[2 ** i]
            convs_seg.append(VolumeRenderResBlock(in_channel, out_channel))
            in_channel = out_channel

        self.convs_seg = nn.Sequential(*convs_seg)

        self.final_conv = VolumeRenderDiscConv2d(in_channel+1, final_out_channel, 2)

        self.stddev_group = 4
        self.stddev_feat = 1

    def cal_stddev(self, x):
    ...
    def forward(self, img, seg):
        out = self.convs_img(img) + self.convs_seg(seg)
        out = self.cal_stddev(out)
...
class ComponentDualBranchDiscriminator(nn.Module):
    def __init__(self, opt):
        super().__init__()
        img_size = opt.renderer_spatial_output_dim
        self.channels = {
            4: 512,
            8: 512,
            16: 512,
        ...
        convs = [VolumeRenderDiscConv2d(3, self.channels[img_size], 1, activate=True)]
        ...
        convs = [VolumeRenderDiscConv2d(1, self.channels[seg_size], 1)]
        ...
        self.final_conv = VolumeRenderDiscConv2d(in_channel+1, self.channels[4], 3)
        self.final_linear = nn.Sequential(
            EqualLinear(self.channels[4] * 2 * 2, self.channels[4], activation='fused_lrelu'),
            EqualLinear(self.channels[4], 1),
        )
        self.class_linear = nn.Sequential(
            EqualLinear(self.channels[4] * 2 * 2, self.channels[4], activation='fused_lrelu'),
            EqualLinear(self.channels[4], opt.n_wcode),
        )
    ...
    def forward(self, img, seg):
        batch = img.shape[0]
        img_component = img * seg
        out = self.convs_img(img_component)
...
class Discriminator(nn.Module):
    def __init__(self, opt, blur_kernel=[1, 3, 3, 1]):
        ...
        ### img
        convs_img = [ConvLayer(3, channels[init_size], 1)]
        ...
        for i in range(log_size, 2, -1):
            ...
            convs_img.append(ResBlock(in_channel, out_channel, blur_kernel))
        ...
        ### seg
        convs_seg = [ConvLayer(opt.n_wcode, channels[init_size], 1)]
        ...
        for i in range(log_size, 2, -1):
            ...
            convs_seg.append(ResBlock(in_channel, out_channel, blur_kernel))
