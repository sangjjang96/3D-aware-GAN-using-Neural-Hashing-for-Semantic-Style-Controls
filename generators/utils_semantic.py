...
class FixedStyledConv(nn.Module):
    ...
    def __init__(
        ...
        self.conv = torch.nn.Conv2d(in_channel, out_channel, kernel_size, bias=None, stride=1, padding=kernel_size // 2, dilation=1, groups=1,)
    ...
    def forward(self, input, style=None, noise=None):
        ...
        if self.upsample:
            ...
            self.conv = self.conv.to(input.device)
            # out = self.conv(input)
...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, skip=None):
        out = self.conv(input, style)
        out = out.to(self.bias.device) + self.bias
