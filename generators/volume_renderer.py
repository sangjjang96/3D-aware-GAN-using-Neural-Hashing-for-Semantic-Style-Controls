...
class SirenGenerator(nn.Module):
    def __init__(self, D=3, W=128, style_dim=256, input_ch=64, input_ch_views=3, output_ch=4,
        ...
        self.mlp = torch.nn.Sequential(torch.nn.Linear(128, 128), torch.nn.LeakyReLU(), torch.nn.Linear(128, 128))
...
class VolumeFeatureRenderer(nn.Module):
    ...
    def volume_integration(self, normalized_pts, viewdirs, styles, raw, z_vals, rays_d, pts, return_eikonal=False, semantics=None):
        ...
        with torch.no_grad():
            input_dirs = viewdirs.unsqueeze(self.samples_dim).repeat(1, 1, 1, normalized_pts.shape[3], 1)
            net_inputs = torch.cat([normalized_pts, input_dirs], self.channel_dim)
            branch_latent = styles[:,:self.depth_total]
            # branch_latent = styles[:, 0]
    ...
    def run_network(self, inputs, viewdirs, styles=None, init=False):
        input_dirs = viewdirs.unsqueeze(self.samples_dim).repeat(1, 1, 1, inputs.shape[3], 1)
        ...
        if init==False:
            ...
            branch_latent = styles[:,:self.depth_total]
            n_outputs.append(self.init_network(net_inputs, styles=branch_latent).unsqueeze(0))      # [1, B, W, H, steps, 5]
